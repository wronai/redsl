...
![version](https://img.shields.io/badge/version-0.1.0-blue) ![python](https://img.shields.io/badge/python-%3E%3D3.11-blue) ![coverage](https://img.shields.io/badge/coverage-unknown-lightgrey) ![functions](https://img.shields.io/badge/functions-1480-green)
> **1480** functions | **206** classes | **253** files | CC̄ = 3.8
...
```
redsl/
├── project    ├── propozycje    ├── nda-wzor    ├── nda-form    ├── email-notifications    ├── polityka-prywatnosci    ├── config-editor    ├── config-api    ├── project    ├── regulamin    ├── app        ├── index        ├── vallm-pre-commit    ├── index        ├── main        ├── main    ├── consciousness_loop├── redsl/    ├── __main__        ├── main        ├── main        ├── cli_awareness        ├── _fixer_utils    ├── complex_code        ├── doctor        ├── _scan_report        ├── doctor_indent_fixers        ├── sumr_planfile/        ├── planfile_bridge        ├── pre-commit-hook        ├── doctor_fstring_fixers    ├── bad_code        ├── _guard_fixers        ├── doctor_fixers        ├── doctor_helpers        ├── main        ├── main        ├── main        ├── main    ├── history        ├── cli_autonomy        ├── cli_doctor        ├── github_source        ├── main        ├── hybrid        ├── _indent_fixers            ├── verdict        ├── batch_pyqual/            ├── utils        ├── batch        ├── doctor_detectors        ├── main        ├── main            ├── models            ├── helpers        ├── autofix/            ├── runner            ├── config_gen            ├── reporting            ├── discovery            ├── hybrid            ├── todo_gen            ├── reporting            ├── pipeline            ├── runner            ├── parsers            ├── utils            ├── discovery            ├── ruff_analyzer        ├── doctor_data            ├── core            ├── extractors            ├── reporter            ├── fix_decisions            ├── mypy_analyzer            ├── bandit_analyzer            ├── ast_analyzer            ├── validator            ├── reporter            ├── analyzer        ├── pyqual/        ├── autonomy_pr/            ├── git_ops        ├── plan_sync        ├── catalog            ├── models        ├── security        ├── scan    ├── config_standard/        ├── store        ├── nlp_handlers        ├── models        ├── paths        ├── agent_bridge        ├── profiles        ├── basic_analysis    ├── examples/        ├── _common        ├── badge        ├── custom_rules            ├── pipeline        ├── multi_project        ├── api_integration        ├── awareness            ├── models    ├── diagnostics/            ├── models    ├── core/        ├── audit        ├── pr_bot        ├── pipeline        ├── review        ├── intent    ├── autonomy/        ├── adaptive_executor        ├── quality_gate        ├── metrics        ├── smart_scorer        ├── perf_bridge    ├── formatters/        ├── growth_control        ├── core        ├── full_pipeline        ├── refactor        ├── debug        ├── cycle        ├── batch        ├── llm_banner        ├── auto_fix        ├── pyqual    ├── config        ├── config        ├── __main__    ├── cli/        ├── scan        ├── examples        ├── utils        ├── model_policy        ├── pyqual_example        ├── debug        ├── batch        ├── resolution        ├── models        ├── planfile    ├── execution/        ├── reporter        ├── sandbox_execution        ├── validation        ├── decision        ├── executor        ├── reflector        ├── base    ├── bridges/        ├── applier        ├── gate        ├── selection/        ├── refactor        ├── registry/        ├── memory_learning        ├── llx_router    ├── llm/            ├── sources/            ├── ops                ├── base            ├── aggregator            ├── metrics            ├── config            ├── checks            ├── selector        ├── direct_types    ├── orchestrator    ├── memory/        ├── direct_guard        ├── direct        ├── scheduler        ├── direct_imports        ├── prompts        ├── engine    ├── refactors/        ├── diff_manager        ├── models        ├── _base    ├── ci/        ├── body_restorer            ├── strategy        ├── direct_constants        ├── github_actions        ├── git_timeline        ├── timeline_toon        ├── timeline_git        ├── ecosystem        ├── timeline_analysis        ├── proactive        ├── change_patterns        ├── self_model        ├── health_model    ├── validation/    ├── awareness/        ├── sandbox        ├── pyqual_bridge            ├── models        ├── tool_check    ├── utils/        ├── json_helpers        ├── regix_bridge        ├── testql_bridge        ├── incremental        ├── sumd_bridge            ├── models    ├── analyzers/        ├── analyzer        ├── python_analyzer        ├── vallm_bridge        ├── metrics        ├── utils        ├── redup_bridge        ├── resolver        ├── semantic_chunker        ├── code2llm_bridge        ├── parsers/        ├── toon_analyzer        ├── radon_analyzer            ├── validation_parser            ├── functions_parser            ├── duplication_parser    ├── integrations/        ├── health_routes        ├── pyqual_routes    ├── api/        ├── webhook        ├── webhook_routes            ├── project_parser        ├── debug_routes        ├── example_routes    ├── dsl/        ├── rule_generator        ├── planfile_updater    ├── main        ├── ast_transformers        ├── quality_visitor        ├── refactor_routes    ├── models        ├── timeline_models        ├── engine        ├── cycle        ├── core        ├── proposals        ├── llm_policy        ├── secrets        ├── models        ├── logging```
...
📦 `redsl.execution`
📄 `redsl.execution.cycle` (13 functions)
📄 `redsl.execution.decision` (9 functions)
📄 `redsl.execution.executor`
📄 `redsl.execution.planfile_updater` (13 functions)
